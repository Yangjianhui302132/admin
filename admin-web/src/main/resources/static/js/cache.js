...
layui.use(['form','jquery',"layer"],function() {
    ...
    if(window.sessionStorage.getItem('userFace') &&  $(".userAvatar").length > 0){
        ...
        $("#userFace").attr("src",Context.path+"/images/face.jpg");
    }
...
function showNotice(){
    layer.open({
        ...
        btn: ['前往'],
        moveType: 1,
        content: '<div style="padding:15px 20px; text-align:justify; line-height: 22px; text-indent:2em;border-bottom:1px solid #e2e2e2;"><p class="layui-red">欢迎使用后台模板</p><p>如有需要，可前往本人github下载源码</p><p></p></div>',
        success: function(layero){
            var btn = layero.find('.layui-layer-btn');
            btn.css('text-align', 'center');
            btn.on("click",function(){
                window.open("https://github.com/Yangjianhui302132/admin");
                tipsShow();
            });
        },
